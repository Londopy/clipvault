...
fn load_icon() -> tray_icon::Icon {
    // icon is compiled into the binary so it works no matter where the exe
    // is run from (loading from a relative assets/ path broke standalone use)
    const ICON: &[u8] = include_bytes!("../assets/icon_32.png");
    ...
    // last resort: solid blue 32x32 square so we don't crash
